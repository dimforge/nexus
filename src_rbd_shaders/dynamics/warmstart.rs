...
/// body-pair matching as the warmstart impulse transfer), between the
/// topo-gc reset and its iterations.
///
/// Contacts persist across frames, so most constraints can reuse last
/// frame's color and the iterations only color the genuinely new ones; the
/// fix-conflicts pass still validates every seed, so a stale seed is simply
/// uncolored and recomputed.
#[spirv_bindgen]
#[spirv(compute(threads(64)))]
